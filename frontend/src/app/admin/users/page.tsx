...
export default function UsersPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    fetchUsers().then(() => { if (cancelled) return; });
    return () => { cancelled = true; };
  }, [fetchUsers]);

  // Debounced search — сброс на первую страницу
  useEffect(() => {
  ...
  const handleAction = async (action: string, user: UserListItem) => {
    ...
    const actionMap: Record<string, 'verify' | 'suspend' | 'ban' | 'activate' | 'unverify'> = {
      suspend: 'suspend', ban: 'ban', activate: 'activate', verify: 'verify',
    };
  ...
  const handleBulkAction = async (action: string) => {
    const actionMap: Record<string, 'verify' | 'suspend' | 'ban' | 'activate'> = {
      verify: 'verify', suspend: 'suspend', ban: 'ban', activate: 'activate',
    };
    const mapped = actionMap[action];
    ...
    <div className={styles.pageContainer}>
      ...
      <div className={styles.headerSection}>
        <div className={styles.headerContent}>
          ...
          <p className={styles.headerDescription}>{totalUsers.toLocaleString()} пользователей</p>
        </div>
      ...
      {loading ? (
        ...
        <GlassCard className="p-12 text-center border-dashed border-[var(--admin-glass-border)]">
          ...
          <p className="text-[var(--admin-text-secondary)]">Попробуйте изменить фильтры или поисковый запрос</p>
        </GlassCard>
      ) : (
        <>
          {viewMode === 'table' ? (
